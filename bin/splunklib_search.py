...
class splunklib_search:
    ...
    def __init__(self, base_url, username, password, auth_token, proxy=None, search_timeout=3600, check_cert=False, logger_name=None, log_level=logging.INFO):
        ...
        if logger_name:
            ...
            logging.basicConfig(format='%(asctime)s, Level=%(levelname)s, Pid=%(process)s, Logger=%(name)s, File=%(filename)s, Line=%(lineno)s, %(message)s')
    ...
    def _authenticate(self, username, password, auth_token):
        try:
            self._service = self._connect(username=username, password=password, token=auth_token, host=self._server, port=self._port, verify=self._verify)
        except Exception as e:
    ...
    def run_search(self, search, max_search_time=3600):
        ...
        kwargs_search = {'output_mode': 'json',
...
def main() -> int:
    ...
    if 'SPL_USERNAME' in os.environ and 'SPL_PASSWORD' in os.environ:
        ...
        logging.error("Store Splunk username and password in env vars for testing: SPL_USERNAME and SPL_PASSWORD")
        logging.error("e.g. export SPL_USERNAME=admin SPL_PASSWORD=your_password_here")
        return 1

    proxy = None
    api_search = splunklib_search(base_url=base_url, username=username, password=password, auth_token=auth_token, proxy=proxy)
